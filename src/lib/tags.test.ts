...
describe("buildTags", () => {
  it("sets app, file name, content type, and file identity", () => {
    const tags = buildTags({ name: "photo.png", type: "image/png" }, "abcDEF");
    ...
    expect(tagValue(tags, "File-SHA256")).toBe("abcdef");
  });

  it("defaults the content type", () => {
    const tags = buildTags({ name: "blob", type: "" }, "aa");
